...
create or replace procedure activity_show(USERID in user_table.user_ID%TYPE)
	IS
...
begin
    ...
    for R in (select activity_name, activity_type from activity)  loop
...
create or replace procedure insert_user_activity(USERID in user_table.user_ID%TYPE)
IS
  ...
  begin
    select activity_ID into ACTIVITYID from activity where activity_name = ACTIVITYNAME;
...
create or replace procedure delete_user_activity(USERID in user_table.user_ID%TYPE)
IS
...
begin
    ...
    activity_ID = (select activity_ID from activity where activity_name = LOWER(ACTIVITYNAME));
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Select option from above menu (1/2/3): '||MENU_OPTION);
    	...
    	IF MENU_OPTION = 1 THEN
            activity_show(USERID);
			insert_user_activity(USERID);
            user_activity_show(USERID);
		ELSIF MENU_OPTION = 2 THEN
			delete_user_activity(USERID);
			user_activity_show(USERID);
		ELSE
