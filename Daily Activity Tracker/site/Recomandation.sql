...
create or replace function weekly_predict(USERID IN user_table.user_ID%TYPE)
  return number
  is
  PERCENTAGE number := 0;
  PERHOUR number := 0;
...
create or replace procedure weekly_recomandation(USERID IN user_table.user_ID%TYPE)
  is
  TOTAL number := 0;
  PERCENTAGE number := 0;
  PERHOUR number := 0;
...
begin
    ...
    select count(activity_date) into EXCOUNT from weekly_activity_table where user_ID = 2 
    ...
    exception
      when no_data_found then
        DBMS_OUTPUT.PUT_LINE('No useful recomandation available.');
