...
create or replace procedure mange_weekly_chart(USERID in user_table.user_ID%TYPE, USERFROM in number)
    IS
...
begin
    ...
    select min(activity_date) into STARTDATE from 
    (select activity_date,user_ID from weekly_activity_table union
    select activity_date,user_ID from weekly_activity_table_site) where user_ID = USERID;
    ...
    LOOP
        ...
        for R in (select activity_type from 
        (select activity_type,user_ID,activity_date from weekly_activity_table union
        select activity_type,user_ID,activity_date from weekly_activity_table_site)
        where user_ID = USERID and activity_date = STARTDATE GROUP BY activity_type) loop
        ...
        for R in (select sum(hour) as PERHOUR from 
        (select hour,activity_type,user_ID,activity_date from weekly_activity_table union
        select hour,activity_type,user_ID,activity_date from weekly_activity_table_site)
        where user_ID = USERID and activity_date = STARTDATE GROUP BY activity_type) loop
    ...
    for R in (select activity_type from 
        (select activity_type,user_ID,activity_date from weekly_activity_table union
        select activity_type,user_ID,activity_date from weekly_activity_table_site)
        where user_ID = USERID  GROUP BY activity_type) loop
    ...
    PRED := weekly_predict(USERID,USERFROM);
    if PRED = 2 then
    ...
    weekly_recomandation(USERID,USERFROM);
    DBMS_OUTPUT.PUT_LINE(chr(10));
